...
router.post('/', authenticate, authorize('customer'), async (req, res, next) => {
  try {
    ...
    let deliveryFee = 0;
    if (delivery_zone_id) {
      const zone = await pool.query('SELECT fee FROM delivery_zones WHERE id = $1 AND shop_id = $2', [delivery_zone_id, shop_id]);
    ...
    const orderResult = await pool.query(
      `INSERT INTO orders (order_number, customer_id, shop_id, subtotal, delivery_fee, platform_commission, total_amount, delivery_address, special_instructions)
       ...
       RETURNING *`,
      [orderNumber, req.user.id, shop_id, subtotal, deliveryFee, platformCommission, totalAmount, delivery_address, special_instructions]
    );
    ...
    await pool.query('UPDATE shops SET total_orders = total_orders + 1 WHERE id = $1', [shop_id]);
